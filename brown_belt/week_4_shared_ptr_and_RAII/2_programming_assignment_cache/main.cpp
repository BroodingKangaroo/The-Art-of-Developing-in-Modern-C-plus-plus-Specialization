...
void TestCaching(const Library& lib) {
    ...
    // не достаточно, чтобы полностью проверить правильность реализации стратегии
    ...
    for (int task_num = 0; task_num < tasks_count; ++task_num) {
        tasks.push_back(async([&cache, &lib, task_num] {
            ...
            for (int i = 0; i < trials_count; ++i) {
                ...
                auto actual_content = cache->GetBook(book_name)->GetContent();
                auto expected_content = lib.content.find(book_name)->second->GetContent();
                ASSERT_EQUAL(
                        actual_content,
                        expected_content
                );
