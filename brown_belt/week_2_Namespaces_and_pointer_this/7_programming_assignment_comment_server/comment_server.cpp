...
class HttpResponse {
public:
    explicit HttpResponse(HttpCode code) : response_code_(code) {};
...
ostream &operator<<(ostream &output, const HttpResponse &resp) {
    string code_message;
    if (resp.response_code_ == HttpCode::Ok) code_message = "200 OK";
    else if (resp.response_code_ == HttpCode::NotFound) code_message = "404 Not found";
    else code_message = "302 Found";
    output << "HTTP/1.1 " << code_message << '\n';

    for (const auto&[key, value]: resp.headers_) {
        output << key << ": " << value << '\n';
    }

    if (!resp.body_.empty()) output << "Content-Length: " << resp.body_.length() << '\n';
    output << '\n' << resp.body_;
    return output;
}
...
public:
    HttpResponse ServeRequest(const HttpRequest &req) {
        HttpResponse response_(HttpCode::Ok);
        if (req.method == "POST") {
            ...
            if (req.path == "/user_comments") {
                ...
                response_.SetContent("What's the answer for The Ultimate Question of Life, the Universe, and Everything?");
            } else {
                response_.SetCode(HttpCode::NotFound);
            }
        }
