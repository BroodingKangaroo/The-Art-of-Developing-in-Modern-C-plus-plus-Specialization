...
ForwardIterator max_element_if(ForwardIterator first, ForwardIterator last, UnaryPredicate pred) {
    ...
    for (auto it = largest; it != last; it++) {
        if (it != largest && pred(*it) && *largest < *it) {
            largest = it;
        }
    }
    return largest;
...
void TestOneElement() {
    const vector<int> one = {2};
    auto IfTwo = [](int element) {
        return element == 2;
    };
    Assert(
            max_element_if(one.begin(), one.end(), IfTwo) == one.begin(),
            "Expect begin of the vector"
    );
}
