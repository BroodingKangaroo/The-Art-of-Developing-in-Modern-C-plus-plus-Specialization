...
public:
    ...
    AirportCounter() {
        for (size_t i = 0; i < static_cast<uint32_t>(TAirport::Last_); i++) {
            dict[i].first = static_cast<TAirport>(i);
        }
    }
    ...
    AirportCounter(TIterator begin, TIterator end):AirportCounter() {
        for (auto it = begin; it < end; ++it) {
            dict[static_cast<uint32_t>(*it)].second++;
        }
    }

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return dict[static_cast<uint32_t>(airport)].second;
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        dict[static_cast<uint32_t>(airport)].second++;
    }
    ...
    void EraseAll(TAirport airport) {
        dict[static_cast<uint32_t>(airport)].second = 0;
    }
...
private:
    array<Item, static_cast<uint32_t>(TAirport::Last_)> dict;
};
